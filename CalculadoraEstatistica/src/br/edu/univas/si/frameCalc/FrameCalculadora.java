...
public class FrameCalculadora extends JFrame{
			...
			radioButtonErro.addChangeListener(new ChangeListener() {
				...
				public void stateChanged(ChangeEvent e) {
					if(radioButtonErro.isSelected()){
						getLabelErroTamanho().setText("Desvio Padrão");
	...
	private void buttonClicked(){
		...
		double erro_ou_desvioPadrao =  Double.valueOf(getSpnErroTamanho().getValue().toString()) / 100;
		double result = 0f;
		float p = 0.5f;
		...
		if(getComboBoxConfiabilidade().getSelectedItem().equals("99")){
			z = 2.57f; //Alinne falou que é pra pegar o z para baixo.
		} 		
		
		if(getRadioButtonErro().isSelected()){
			//CALCULA ERRO DA AMOSTRA
			result = (z*erro_ou_desvioPadrao) / (Math.sqrt(populacao));
			
		}else{
			//CALCULA TAMANHO DA AMOSTRA  247
			dividendo = ((z*z)	* (p*(1-p)))/(erro_ou_desvioPadrao*erro_ou_desvioPadrao);
			divisor = 1 + (((z*z)*(p*(1-p)))/((erro_ou_desvioPadrao*erro_ou_desvioPadrao)*populacao));
